...
window.lCalendar = (function() {
	var MobileCalendar = function() {
		...
		this.maxm = 59, // 最大分钟
		this.maxS = 59,  // 最大秒
		this.access_max_year = false // 达到年最大值
		this.access_min_year = false // 达到年最小值
		this.access_max_month = false
		this.access_min_month = false
		this.access_max_day = false
		this.access_min_day = false
		this.access_max_hour = false
		this.access_min_hour = false
		this.access_max_minute= false
		this.access_min_minute = false
		this.aceess_max_second = false
		this.access_min_second = false
	}
	MobileCalendar.prototype = {
		...
		set:function(start,end){
			...
			var minArr = startArr[0].split('-')
				this.minY = ~~minArr[0];
				this.minM = ~~minArr[1];
				this.minD = ~~minArr[2];

			var maxTimeArr = startArr[1].split(':')
				this.maxH = ~~maxTimeArr[0]
				this.maxm = ~~maxTimeArr[1]
				this.maxS = ~~maxTimeArr[2]

				var maxArr = end.split(' ')[0].split('-');
				this.maxY = ~~maxArr[0];
				this.maxM = ~~maxArr[1];
				this.maxD = ~~maxArr[2];
        ...
        var rs;
			...
			**/
			function setDateGearTooth() {
				...
				if (date_mm && date_mm.getAttribute("val")) {
					...
					var maxM = 11;
					var minM = 0;
					_self.access_min_year = false
					_self.access_max_year = false
					//当年份到达最大值
					if (yyVal == passY - 1) {
						maxM = _self.maxM - 1;
						_self.access_max_year = true
					}
					//当年份到达最小值
					if (yyVal == 0) {
						minM = _self.minM - 1;
						_self.access_min_year = true
					}
					//p 当前节点前后需要展示的节点个数
				...
				if (date_dd && date_dd.getAttribute("val")) {
					...
					var minD = 0;
					_self.access_max_month = false
					_self.access_min_month = false
					//当年份月份到达最大值
					if (yyVal == passY - 1 && _self.maxM == mmVal + 1) {
						maxD = _self.maxD - 1;
						_self.access_max_month = true
					}
					//当年、月到达最小值
					if (yyVal == 0 && _self.minM == mmVal + 1) {
						minD = _self.minD - 1;
						_self.access_min_month = true
					}
					for (var p = 0; p < maxD - minD + 1; p++) {
						itemStr += "<div class='tooth'>" + (minD + p + 1) + "</div>";
					}
					date_dd.innerHTML = itemStr;
					_self.access_max_day = false
					_self.access_min_day = false

					if (ddVal >= maxD) {
						ddVal = maxD;
						_self.access_max_day = true
						date_dd.setAttribute("val", ddVal);
					} else if (ddVal <= minD) {
						ddVal = minD;
						_self.access_min_day = true
						date_dd.setAttribute("val", ddVal);
					}
			...
			*/
			function setTimeGearTooth() {
				// console.log('我来重置时间选择器了。。。',_self)
				var time_hh = _self.gearDate.querySelector(".time_hh");
				if (time_hh && time_hh.getAttribute("val")) {
					var i = "", min_h=0,max_h=23;
					var hhVal = parseInt(time_hh.getAttribute("val"));
					let translateCount = hhVal

					// 如果年月日都到最大值，时 也只能在最大最小之间
					if(_self.access_max_year && _self.access_max_month && _self.access_max_day){
						min_h = _self.minH;
						max_h = _self.maxH;
						if(hhVal>=_self.maxH){
							hhVal = _self.maxH
						...
						translateCount = hhVal
						time_hh.setAttribute("val", hhVal);

						// console.log('年月日都到了 最大值', hhVal, min_h, max_h, _self.maxH);
					}else{
						max_h = 23
						min_h = 0
					}
					for (var g = min_h; g <= max_h; g++) {
						i += "<div class='tooth'>" + g + "</div>";
					}
					time_hh.innerHTML = i;
					
					time_hh.style["-webkit-transform"] = 'translate3d(0,' + (8 - (translateCount) * 2) + 'em,0)';
					time_hh.setAttribute('top', 8 - (translateCount) * 2 + 'em');
				} else {
					return
				}
				var time_mm = _self.gearDate.querySelector(".time_mm");
				var mmVal = parseInt(time_mm.getAttribute("val"));
				var translateCount = mmVal
				var hhVal = time_hh.getAttribute("val")
				// console.log('当前小时，最大小时',hhVal,_self.maxH)
				var min_mm = 0,max_mm = 59
				_self.access_max_hour = false
				// 小时达到最大
				if(hhVal>=_self.maxH){
					_self.access_max_hour = true
				}
				if(_self.access_max_year&&_self.access_max_month&&_self.access_max_day&&_self.access_max_hour){
					max_mm = _self.maxm
					if(mmVal>=_self.maxm){
							mmVal = _self.maxm
							translateCount = mmVal
				...
				if (time_mm && time_mm.getAttribute("val")) {
					...
					for (var g = min_mm; g <= max_mm; g++) {
						i += "<div class='tooth'>" + g + "</div>";
					}
					time_mm.innerHTML = i;
					time_mm.style["-webkit-transform"] = 'translate3d(0,' + (8 -(translateCount) * 2) + 'em,0)';
					time_mm.setAttribute('top', 8 - (translateCount) * 2 + 'em');
				} else {
					return
				}
			...
			**/
				...
				target["int_" + target.id] = setInterval(function() {
					...
					switch (target.dataset.datetype) {
						...
						case "date_dd":
							...
							break;
						case "time_hh":
							var maxHH = 23,minHH=0;
							if(_self.access_max_year && _self.access_max_month &&_self.access_max_day){
								maxHH = _self.maxH
							}
							var minTop = 8 - (maxHH - minHH) * 2;
							if (pos < minTop) {
								pos = minTop;// -38
								stopGear = true;
							}
							...
							break;
						case "time_mm":
						var max_mm = 59,min_mm=0;
							if(_self.access_max_year && _self.access_max_month &&_self.access_max_day && _self.access_max_hour){
								max_mm = _self.maxm
							}
							var minTop = 8 - (max_mm - min_mm) * 2;
							if (pos < minTop) {
								pos = minTop;// -110
								stopGear = true;
							}
			...
			function setGear(target, val) {
				...
				// console.log('设置值了' , target.dataset.datetype, val)
			...
			function finishMobileDate(e) {
				...
				typeof _self.submitCallBack == "function" && _self.submitCallBack(_self.trigger.value)
			}
			//日期时间确认
